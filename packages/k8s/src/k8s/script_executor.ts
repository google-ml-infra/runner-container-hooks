...
export namespace script_executor {
    export class ScriptRequest extends pb_1.Message {
        ...
        serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
            ...
            if (this.script.length)
                writer.writeString(1, this.script);
            if (this.id.length)
        ...
        static deserialize(bytes: Uint8Array | pb_1.BinaryReader): ScriptRequest {
            const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes), message = new ScriptRequest();
    ...
    export class ScriptResponse extends pb_1.Message {
        ...
        static deserialize(bytes: Uint8Array | pb_1.BinaryReader): ScriptResponse {
            const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes), message = new ScriptResponse();
            while (reader.nextField()) {
                if (reader.isEndGroup())
                    break;
                switch (reader.getFieldNumber()) {
    ...
    export abstract class UnimplementedScriptExecutorService {
        static definition = {
            ExecuteScript: {
                ...
                requestStream: false,
                responseStream: true,
                requestSerialize: (message: ScriptRequest) => Buffer.from(message.serialize()),
                requestDeserialize: (bytes: Buffer) => ScriptRequest.deserialize(new Uint8Array(bytes)),
                responseSerialize: (message: ScriptResponse) => Buffer.from(message.serialize()),
                responseDeserialize: (bytes: Buffer) => ScriptResponse.deserialize(new Uint8Array(bytes))
            }
        };
        [method: string]: grpc_1.UntypedHandleCall;
        abstract ExecuteScript(call: grpc_1.ServerWritableStream<ScriptRequest, ScriptResponse>): void;
    }
    export class ScriptExecutorClient extends grpc_1.makeGenericClientConstructor(UnimplementedScriptExecutorService.definition, "ScriptExecutor", {}) {
        constructor(address: string, credentials: grpc_1.ChannelCredentials, options?: Partial<grpc_1.ChannelOptions>) {
            super(address, credentials, options);
        }
        ExecuteScript: GrpcStreamServiceInterface<ScriptRequest, ScriptResponse> = (message: ScriptRequest, metadata?: grpc_1.Metadata | grpc_1.CallOptions, options?: grpc_1.CallOptions): grpc_1.ClientReadableStream<ScriptResponse> => {
